...
contract LUMENVaultDistributorBacker is Ownable2StepUpgradeable {
    ...
    function initialize(ILUMENVault _vault, TPoolInfo[] calldata _pools) external initializer {
    ...
    function distribute(TExpectedSwap[] memory swaps, uint256 lumenUsdtPrice) external onlyOwner {
        ...
        for (uint i = 0; i < poolCount; i++) {
            ...
            ILUMENVaultDistributor(pools[i].distributor).addReward(THE, address(this), theAmount);
            ...
            if (pools[i].bonusToken == USDT) {
                ...
                ILUMENVaultDistributor(pools[i].distributor).addReward(USDT, address(this), usdtAmount);
                continue;
            }
            if (pools[i].bonusToken == LUMEN) {
                swapToLUMEN(usdtAmount, lumenUsdtPrice);
                uint lumenBalance = IERC20(LUMEN).balanceOf(address(this));
                IERC20(LUMEN).approve(pools[i].distributor, lumenBalance);
                ILUMENVaultDistributor(pools[i].distributor).addReward(LUMEN, address(this), lumenBalance);
                continue;
            }

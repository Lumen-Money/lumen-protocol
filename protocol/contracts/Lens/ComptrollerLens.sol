...
contract ComptrollerLens is ComptrollerLensInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     * @param luTokenBorrowed Address of the borrowed luToken
     * @param luTokenCollateral Address of collateral for the borrow
    ...
    function liquidateCalculateSeizeTokens(
         ...
         */
        uint exchangeRateMantissa = LuToken(luTokenCollateral).exchangeRateStored(); // Note: reverts on error
        uint seizeTokens;
    ...
    /**
     * @notice Computes the number of LUMUSD tokens to be seized in a liquidation event
     * @param comptroller Address of comptroller
     * @param luTokenCollateral Address of collateral for luToken
    ...
    function liquidateLUMUSDCalculateSeizeTokens(
        ...
        /* Read oracle prices for borrowed and collateral markets */
        uint priceBorrowedMantissa = 1e18; // Note: this is LUMUSD
        uint priceCollateralMantissa = ComptrollerInterface(comptroller).oracle().getUnderlyingPrice(
            LuToken(luTokenCollateral)
        );
     ...
     * @param comptroller Address of comptroller
     * @param account Address of the borrowed luToken
     * @param luTokenModify Address of collateral for luToken
     * @param redeemTokens Number of luTokens being redeemed
    ...
    function getHypotheticalAccountLiquidity(
        ...
        LuToken[] memory assets = ComptrollerInterface(comptroller).getAssetsIn(account);
        uint assetsCount = assets.length;
        for (uint i = 0; i < assetsCount; ++i) {
            LuToken asset = assets[i];

            // Read the balances and exchange rate from the luToken
            (oErr, vars.luTokenBalance, vars.borrowBalance, vars.exchangeRateMantissa) = asset.getAccountSnapshot(
                account
            );
            if (oErr != 0) {
            ...
            // sumCollateral += tokensToDenom * luTokenBalance
            vars.sumCollateral = mul_ScalarTruncateAddUInt(vars.tokensToDenom, vars.luTokenBalance, vars.sumCollateral);
            ...
            // Calculate effects of interacting with luTokenModify
            if (asset == luTokenModify) {
                // redeem effect
        ...
        LUMUSDControllerInterface lumUsdController = ComptrollerInterface(comptroller).lumUsdController();

        if (address(lumUsdController) != address(0)) {
            vars.sumBorrowPlusEffects = add_(vars.sumBorrowPlusEffects, lumUsdController.getLUMUSDRepayAmount(account));
        }

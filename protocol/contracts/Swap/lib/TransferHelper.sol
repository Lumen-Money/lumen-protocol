...
library TransferHelper {
    ...
    /**
     * @dev Transfer `value` amount of `NEON` from the calling contract to `to`. If `token` returns no value,
     * non-reverting calls are assumed to be successful.
     * @param to Address of the receiver
     * @param value Amount need to transfer
     */
    function safeTransferNEON(address to, uint256 value) internal {
        (bool success, ) = to.call{ value: value }(new bytes(0));
        if (!success) {
            revert SafeTransferNEONFailed();
        }
    }
}

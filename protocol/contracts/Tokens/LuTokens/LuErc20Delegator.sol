...
/**
 * @title LmnFi's LuErc20Delegator Contract
 * @notice luTokens which wrap an EIP-20 underlying and delegate to an implementation
 * @author LmnFi
 */
contract LuErc20Delegator is LuTokenInterface, LuErc20Interface, LuDelegatorInterface {
    /**
    ...
    fallback() external payable {
        require(msg.value == 0, "LuErc20Delegator:fallback: cannot send value to fallback");
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemTokens The number of luTokens to redeem into underlying asset
     * @return uint Returns 0 on success, otherwise returns a failure code (see ErrorReporter.sol for details).
     */
    ...
    function liquidateBorrow(
        ...
        bytes memory data = delegateToImplementation(
            abi.encodeWithSignature("liquidateBorrow(address,uint256,address)", borrower, repayAmount, luTokenCollateral)
        );
        return abi.decode(data, (uint));
    }
     ...
     *  It's absolutely critical to use msg.sender as the borrowed luToken and not a parameter.
    ...
    function _setImplementation(
        ...
        require(msg.sender == admin, "LuErc20Delegator::_setImplementation: Caller must be admin");

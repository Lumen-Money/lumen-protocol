...
import "./LuTokenInterfaces.sol";

/**
 * @title LmnFi's luToken Contract
 * @notice Abstract base for luTokens
 * @author LmnFi
 */
abstract contract LuToken is LuTokenInterface, Exponential, TokenErrorReporter {
     ...
     * @dev Will fail unless called by another luToken during the process of liquidation.
     *  Its absolutely critical to use msg.sender as the borrowed luToken and not a parameter.
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of luTokens to seize
    ...
    function getAccountSnapshot(address account) override external view returns (uint, uint, uint, uint) {
        uint luTokenBalance = accountTokens[account];
        uint borrowBalance;
        uint exchangeRateMantissa;
        ...
        return (uint(Error.NO_ERROR), luTokenBalance, borrowBalance, exchangeRateMantissa);
    }

    /**
     * @notice Returns the current per-block supply interest rate for this luToken
    ...
    /**
     * @notice Returns the current per-block borrow interest rate for this luToken
    ...
    /**
     * @notice Get cash balance of this luToken in the underlying asset
    ...
    /**
     * @notice Calculates the exchange rate from the underlying to the LuToken
    ...
    /**
     * @notice User supplies assets into the market and receives luTokens in exchange
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
         ...
         *  Note: The luToken must handle variations between ERC-20 and NEON underlying.
         *  `doTransferIn` reverts if anything goes wrong, since we can't be sure if
         *  side-effects occurred. The function returns the amount actually transferred,
         *  in case of a fee. On success, the luToken holds an additional `actualMintAmount`
         *  of cash.
         */
        vars.actualMintAmount = doTransferIn(minter, mintAmount);

        /*
         * We get the current exchange rate and calculate the number of luTokens to be minted:
        ...
        /*
         * We calculate the new total supply of luTokens and minter token balance, checking for overflow:
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver The address of the account which is receiving the luTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    ...
    /**
     * @notice Payer supplies assets into the market and receiver receives luTokens in exchange
     * @dev Assumes interest has already been accrued up to the current block
     * @param payer The address of the account which is paying the underlying token
     * @param receiver The address of the account which is receiving luToken
     * @param mintAmount The amount of the underlying asset to supply
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintBehalfFresh(address payer, address receiver, uint mintAmount) internal returns (uint, uint) {
        ...
        /*
         * We calculate the new total supply of luTokens and receiver token balance, checking for overflow:
         *  totalSupplyNew = totalSupply + mintTokens
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemAmount The amount of underlying to receive from redeeming luTokens
     * @return uint Returns 0 on success, otherwise returns a failure code (see ErrorReporter.sol for details).
     */
    ...
    /**
     * @notice User redeems luTokens in exchange for the underlying asset
     * @dev Assumes interest has already been accrued up to the current block
     * @param redeemer The address of the account which is redeeming the tokens
     * @param redeemTokensIn The number of luTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @param redeemAmountIn The number of underlying tokens to receive from redeeming luTokens (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @return uint Returns 0 on success, otherwise returns a failure code (see ErrorReporter.sol for details).
     */
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
         ...
         *  Note: The luToken must handle variations between ERC-20 and NEON underlying.
         *  On success, the luToken has redeemAmount less of cash.
    ...
    function borrowFresh(address borrower, address payable receiver, uint borrowAmount) internal returns (uint) {
         ...
         *  Note: The luToken must handle variations between ERC-20 and NEON underlying.
         *  On success, the luToken borrowAmount less of cash.
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
         ...
         *  Note: The luToken must handle variations between ERC-20 and NEON underlying.
         *  On success, the luToken holds an additional repayAmount of cash.
    ...
    function liquidateBorrowInternal(
        ...
        error = luTokenCollateral.accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted liquidation failed
        ...
        return liquidateBorrowFresh(msg.sender, borrower, repayAmount, luTokenCollateral);
    }
    ...
    function liquidateBorrowFresh(
        ...
        require(luTokenCollateral.balanceOf(borrower) >= seizeTokens, "LIQUIDATE_SEIZE_TOO_MUCH");

        // If this is also the collateral, run seizeInternal to avoid re-entrancy, otherwise make an external call
        uint seizeError;
        if (address(luTokenCollateral) == address(this)) {
        ...
        emit LiquidateBorrow(liquidator, borrower, actualRepayAmount, address(luTokenCollateral), seizeTokens);
    ...
    /**
     * @notice Transfers collateral tokens (this market) to the liquidator.
     * @dev Called only during an in-kind liquidation, or by liquidateBorrow during the liquidation of another luToken.
     *  Its absolutely critical to use msg.sender as the seizer luToken and not a parameter.
     * @param seizerToken The contract seizing the collateral (i.e. borrowed luToken)
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of luTokens to seize
     * @return uint Returns 0 on success, otherwise returns a failure code (see ErrorReporter.sol for details).
    ...
    function _addReservesFresh(uint addAmount) internal returns (uint, uint) {
         ...
         *  Note: The luToken must handle variations between ERC-20 and NEON underlying.
         *  On success, the luToken holds an additional addAmount of cash.
         *  doTransferIn reverts if anything goes wrong, since we can't be sure if side effects occurred.
         *  it returns the amount actually transferred, in case of a fee.
         */

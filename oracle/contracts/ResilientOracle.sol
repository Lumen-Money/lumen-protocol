 ...
 *
 * For every market (luToken) we configure the main, pivot and fallback oracles. The oracles are configured per
 * luToken's underlying asset address. The main oracle oracle is the most trustworthy price source, the pivot
...
contract ResilientOracle is PausableUpgradeable, AccessControlledV8, ResilientOracleInterface {
    ...
    /// @notice luNEON address
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    address public luNeon;
    ...
    function setSeTokens (address luNeonAddress, address SEUSDAddress) external onlyOwner {
        luNeon = luNeonAddress;
        SEUSD = SEUSDAddress;
    }
     ...
     * @param luToken luToken address
     */
    function updatePrice(address luToken) external override {
        address asset = _getUnderlyingAsset(luToken);
    ...
    /**
     * @notice Gets price of the underlying asset for a given luToken. Validation flow:
     ...
     */
    function getUnderlyingPrice(address luToken) external view override returns (uint256) {
    ...
    /**
     * @dev This function returns the underlying asset of a luToken
     * @param luToken luToken address
     * @return asset underlying asset address
     */
    function _getUnderlyingAsset(address luToken) private view returns (address asset) {
        if (address(luToken) == luNeon) {
            asset = NEON_ADDR;
        } else if (address(luToken) == SEUSD) {
            asset = SEUSD;
        } else {
            asset = LuErc20Interface(luToken).underlying();
        }
    }
}
